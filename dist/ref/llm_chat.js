...
export class LLMChatPipeline {
    constructor(tvm, tokenizer, config) {
        ...
        this.params = this.tvm.detachFromCurrentScope(this.tvm.getParamsFromCache("param", -1));
     ...
     */
    runtimeStatsText() {
        return (`prefill: ${(this.prefillTotalTokens / this.prefillTotalTime).toFixed(4)} tokens/sec, ` +
    ...
    async prefillStep(inp) {
        ...
        const logits = this.tvm.detachFromCurrentScope(this.forward(inputData, newSeqLen));
        this.filledKVCacheLength = newSeqLen;
        this.tvm.endScope();
        const nextToken = await this.sampleTokenFromLogits(logits, this.config.temperature, this.config.top_p);
        logits.dispose();
    ...
    async decodeStep() {
        ...
        const logits = this.tvm.detachFromCurrentScope(this.forward(inputData, this.filledKVCacheLength + 1));
    ...
    async sampleTokenFromLogits(logitsOnGPU, temperature, top_p) {
        this.tvm.beginScope();
        ...
        if (this.config.repetition_penalty < 1.0 + 1e-6) {
            ...
            this.tvm.beginScope();
            const appeared_tokens_ndarray = this.tvm.empty([1, this.appearedTokens.size], "int32", this.tvm.cpu());
            appeared_tokens_ndarray.copyFrom(Array.from(this.appearedTokens));
            this.tvm.applyRepetitionPenalty(this.logitsOnCPU, appeared_tokens_ndarray, this.config.repetition_penalty);
            this.tvm.endScope();
    ...
    async evaluate() {
        ...
        const msg = (`prefill-time=${((decodingStart - prefillStart) / 1000).toFixed(4)} sec` +
